...
public class Thm7InequalityGenerator extends BaseInequalityGenerator {

	public Thm7InequalityGenerator(int filas, int columnas) {
					...
					for (int j3 = j1 + 1; j3 < columnas; j3++)
						for (int i3 = i2; i3 < filas; i3++)
							for (int j2 = j3 + 1; j2 < columnas; j2++) {
								...
								if (i2 - i1 <= 0 || j2 - j3 <= 0 || i3 - i2 <= 0 || j3 - j1 <= 0)
									continue;

								// q1 es el primer -1
								int q1f = i1 + r.nextInt(i2 - i1);
								int q1c = j3 + 1 + r.nextInt(j2 - j3);
								...
								// q3 es el tercer -1
								int q3f = i2 + 1 + r.nextInt(i3 - i2);
								int q3c = j1 + r.nextInt(j3 - j1);
